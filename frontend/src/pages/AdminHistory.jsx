...
const AdminHistory = () => {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      // Added a timestamp to the URL to prevent browser caching of old data
      const response = await axios.get(`http://localhost:5000/api/admin-orders?t=${Date.now()}`);
      
      // 1. Only show Completed or Cancelled
      // 2. FORCED SORT: (b.id - a.id) ensures the newest IDs are always at the top
      const historyOnly = response.data
  ...
  const displayedOrders = orders.filter(o => {
    ...
    // IMPROVED DATE MATCH: Handles both service_date and created_at formats
    const matchesDate = filterDate === '' || 
      (o.service_date && o.service_date.startsWith(filterDate)) ||
      (o.created_at && o.created_at.startsWith(filterDate));

    return matchesSearch && matchesDate;
  });

  if (loading) return <div className="pt-40 text-center font-black text-[#0054a6] animate-pulse uppercase tracking-widest">Accessing Archives...</div>;
    ...
    <div className="pt-32 pb-12 px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw] bg-gray-50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
        <div>
            ...
            <p className="text-gray-400 font-bold mt-2 uppercase text-[10px] tracking-widest">Total Archived Records: {orders.length}</p>
        </div>
        ...
        <div className="flex gap-4 w-full md:w-auto">
            <input 
              ...
              className="bg-white border border-gray-100 px-6 py-4 rounded-2xl text-sm font-bold shadow-sm focus:border-[#0054a6] outline-none w-full md:w-64 transition-all focus:ring-2 focus:ring-blue-100"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <input 
              type="date" 
              className="bg-white border border-gray-100 px-6 py-4 rounded-2xl text-sm font-bold shadow-sm focus:border-[#0054a6] outline-none transition-all focus:ring-2 focus:ring-blue-100"
              value={filterDate}
      ...
      <div className="grid grid-cols-1 gap-4">
        {displayedOrders.length > 0 ? displayedOrders.map((order) => {
          // Safety parsing for services JSON
          let services = [];
          try {
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <span className="text-[10px] font-black text-gray-400 uppercase">
                        Job Date: {order.service_date ? order.service_date.split('T')[0] : order.created_at?.split('T')[0]}
                    </span>
                  </div>

                  <h3 className="text-xl font-black text-gray-900 uppercase mb-4 tracking-tight">{order.customer_name}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-xs font-bold text-gray-600 italic bg-gray-50 p-3 rounded-xl border border-gray-100 inline-block min-w-full">
                    ...
                    {order.status === 'Cancelled' && order.notes && (
                      <div>
                        ...
                        <p className="text-xs text-red-700 font-bold italic bg-red-50 p-3 rounded-xl border border-red-100 inline-block min-w-full">
                            "{order.notes}"
                        </p>
                      </div>
                    )}
          ...
          <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-gray-200 text-gray-400 font-bold uppercase tracking-widest">
            No archived records found matching criteria
          </div>
        )}
      </div>
    </div>
